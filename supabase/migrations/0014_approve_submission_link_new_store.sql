-- 新規店舗としての承認では、承認後にどの店舗が生まれたのか
-- store_submissions側から辿れなかった（target_store_idが修正提案の
-- ときしか設定されていなかった）ため、新規作成時もtarget_store_idに
-- 生成されたstoreのidを設定するようにする。
-- これにより「投稿一覧→承認済み→実際に登録された店舗」を常にたどれる。
...
begin
  ...
  if s.target_store_id is not null then
    ...
    insert into stores (
      ...
      s.name, s.region_id, s.prefecture, s.town, coalesce(s.village, ''), s.address,
