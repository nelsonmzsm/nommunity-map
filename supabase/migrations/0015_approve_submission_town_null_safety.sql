-- 投稿フォームで「海外」を選べるようにし、その場合はtownを未入力（null）で
-- 送信できるようにしたが、stores.townはnot nullのため、villageのときと同様に
-- 新規店舗としての承認がNOT NULL制約違反で失敗する。coalesceで空文字にする。
...
begin
  ...
  if s.target_store_id is not null then
    ...
    insert into stores (
      ...
      s.name, s.region_id, s.prefecture, coalesce(s.town, ''), coalesce(s.village, ''), s.address,
